...
class Caption_Generator():
    ...
    def __init__(self, config, data=None):
        ...
        if self.mode == 'test':
            ...
            self.idxtow = dict(list(zip(list(self.wtoidx.values()), list(self.wtoidx.keys()))))
            self.model()
    ...
    def generate_mask(self, ID_batch, wtoidx):
        nonpadded = [len(
            ID_batch[0]) - x.count(wtoidx["<PAD>"]) for x in ID_batch.tolist()]
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.current_epoch, self.nb_epochs):
                ...
                for batch_idx in range(self.num_batch):
                    batch_features, batch_Ids = next(batch_iter)
